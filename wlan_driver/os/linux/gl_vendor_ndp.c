...
nanNdpInitiatorReqHandler(struct GLUE_INFO *prGlueInfo, struct nlattr **tb) {
	...
	uint8_t aucServiceName[NAN_MAX_SERVICE_NAME_LEN];
	...
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_CONFIG_SECURITY]) {
		...
		if (rNanCmdDataRequest.ucSecurity &&
			...
			kalMemCopy(aucServiceName,
				nla_data(tb[MTK_WLAN_VENDOR_ATTR_NDP_SERVICE_NAME]),
				nla_len(tb[MTK_WLAN_VENDOR_ATTR_NDP_SERVICE_NAME])
			);
			if (nla_len(tb[MTK_WLAN_VENDOR_ATTR_NDP_SERVICE_NAME]) < NAN_MAX_SERVICE_NAME_LEN)
				aucServiceName[nla_len(tb[MTK_WLAN_VENDOR_ATTR_NDP_SERVICE_NAME])] = '\0';
			nanSetNdpPmkid(
